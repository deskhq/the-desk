...
- **The title is enforced mechanically.** The `pr-title` job in `.github/workflows/commitlint.yml` (`amannn/action-semantic-pull-request`) fails the build on a non-Conventional title, and re-runs on `edited` so fixing the title turns it green without a new commit. Its type list and subject rules mirror `commitlint.config.mjs`, and `tests/Unit/PullRequestTitleCheckTest.php` fails if the two ever drift — so update both together, never one alone. In particular the subject must **not** start with a capital letter (that covers acronym-led subjects too: `feat: SSO login…` is rejected, `feat: re-check the SSO policy` is fine), must not end with a full stop, and must be **at most 90 characters** — the bound that keeps the squash header inside commitlint's 100-character `header-max-length`. That length rule is the one you cannot fix after the fact: `header-max-length` is a warning in `commitlint.config.mjs` precisely because an over-long subject that reaches `develop` is history, and as an error it would fail every promotion PR that carries it (#891).
